 ...
 *
 * Priority:
 * 1. Explicit `AWS_BEARER_TOKEN_BEDROCK` env var (manual API key from AWS Console)
 * 2. SigV4-derived token via `@aws/bedrock-token-generator` from IAM credentials
 *    (called separately in `generateBearerTokenFromIam`)
 */
...
export async function generateBearerTokenFromIam(params: {
  ...
  try {
    ...
    // Package not installed or credentials unavailable — silent fallback
    return undefined;
  }
}
 ...
 * 2. IAM credentials (instance role, SSO, access keys) → generate bearer token
...
export async function resolveImplicitMantleProvider(params: {
  ...
  if (!bearerToken) {
    bearerToken = await generateBearerTokenFromIam({ region, env });
    if (bearerToken) {
