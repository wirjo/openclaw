...
describe("bedrock mantle discovery", () => {
  ...
  it("returns null when no auth is available and token generator fails", async () => {
    // With empty env AND no IMDS/token generator, should return null.
    // On instances with IMDS, the token generator may succeed — that's fine,
    // just verify it doesn't throw.
    const provider = await resolveImplicitMantleProvider({
      env: {} as NodeJS.ProcessEnv,
    });

    // Either null (no creds) or a valid provider (IMDS creds found)
    if (provider) {
