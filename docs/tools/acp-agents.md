...
| Value  | Behavior                                                                        |
| ------ | ------------------------------------------------------------------------------- |
| `deny` | Silently deny the permission and continue (graceful degradation). **(default)** |
| `fail` | Abort the session with `AcpRuntimeError`.                                       |
...
> **Note:** OpenClaw defaults to `permissionMode=approve-reads` and `nonInteractivePermissions=deny`. The embedded acpx runtime always runs inside the headless gateway process, so there is never an attached TTY to approve a write/exec prompt — for every ACP session, including ones bound to an interactive channel thread. With the default `deny`, any write or exec that would otherwise trigger a permission prompt is denied immediately instead of raising `AcpRuntimeError: Permission prompt unavailable in non-interactive mode` after a long stall.
>
> If you explicitly want sessions to hard-abort instead of gracefully denying blocked operations, set `nonInteractivePermissions` to `fail`. If you need full write/exec access for harnessed or unattended runs, set `permissionMode` to `approve-all` (the harness-level break-glass switch).
...
| `AcpRuntimeError: Permission prompt unavailable in non-interactive mode`    | `nonInteractivePermissions=fail` explicitly configured (not the default), or a custom `permissionMode`/`nonInteractivePermissions` override blocking writes/exec. | Set `plugins.entries.acpx.config.nonInteractivePermissions` to `deny` (the default) for graceful denial, or `plugins.entries.acpx.config.permissionMode` to `approve-all` for full access, then restart gateway. See [Permission configuration](#permission-configuration). |
| ACP session fails early with little output                                  | Permission prompts are blocked by `permissionMode`/`nonInteractivePermissions`.                                                                                   | Check gateway logs for `AcpRuntimeError`. For full permissions, set `permissionMode=approve-all`; for graceful degradation (default), leave `nonInteractivePermissions=deny`.                                                                                               |
