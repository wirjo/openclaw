...
const MODELS: Record<string, ModelSpec> = {
  "amazon.titan-embed-text-v2:0":              { maxTokens: 8192,   dims: 1024, validDims: [256, 512, 1024], family: "titan-v2" },
  ...
  "cohere.embed-v4:0":                         { maxTokens: 128000, dims: 1536, validDims: [256, 384, 512, 768, 1024, 1536], family: "cohere-v4" },
  "amazon.nova-2-multimodal-embeddings-v1:0":  { maxTokens: 8192,   dims: 1024, validDims: [256, 384, 1024, 3072], family: "nova" },
...
interface AwsSdk {
  ...
  InvokeModelCommand: new (input: {
    modelId: string; body: string; contentType: string; accept: string;
  }) => SdkCommand;
}
...
function buildBody(family: Family, text: string, dims?: number): string {
  switch (family) {
    case "titan-v2": {
      ...
      if (dims != null) { b.dimensions = dims; b.normalize = true; }
...
function parseSingle(family: Family, raw: string): number[] {
  ...
  switch (family) {
    ...
    case "twelvelabs": return data.data?.[0]?.embedding ?? [];
...
export async function createBedrockEmbeddingProvider(
  ...
  debugEmbeddingsLog("memory embeddings: bedrock client", {
    region: client.region, model: client.model, dimensions: client.dimensions, family,
  });

  const invoke = async (body: string): Promise<string> => {
    const res = await sdk.send(new InvokeModelCommand({
      modelId: client.model, body, contentType: "application/json", accept: "application/json",
    }));
  ...
  return {
    provider: { id: "bedrock", model: client.model, maxInputTokens: spec?.maxTokens, embedQuery, embedBatch },
    client,
  };
}
...
export function hasAwsCredentials(env: NodeJS.ProcessEnv = process.env): boolean {
  if (env.AWS_ACCESS_KEY_ID?.trim() && env.AWS_SECRET_ACCESS_KEY?.trim()) return true;
  return CREDENTIAL_ENV_VARS.some((k) => env[k]?.trim());
}
